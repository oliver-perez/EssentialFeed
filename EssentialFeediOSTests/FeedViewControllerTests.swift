...
class FeedViewControllerTests: XCTestCase {
  ...
  func test_loadingFeedInidicator_isVisibleWhileLoadingFeed() {
    let (sut, loader) = makeSUT()
  ...
  func test_loadFeedCompletion_rendersSuccessfullyLoadedFeed() {
    ...
    XCTAssertEqual(sut.numberOfRendererFeedImageViews(), .zero)
  ...
  func test_feedImageView_loadsImageURLWhenNotVisibleAnymore() {
    let image0 = makeImage(url: URL(string: "http://url-0.com")!)
    let image1 = makeImage(url: URL(string: "http://url-1.com")!)
    let (sut, loader) = makeSUT()
  ...
  func test_feedImageView_rendersImageLoadedFromURL() {
    ...
    let imageData0: Data = UIImage.make(withColor: .red).pngData()!
    loader.completeImageLoading(with: imageData0, at: 0)
    XCTAssertEqual(view0?.renderedImage, imageData0, "Expected image for first view once first image loading completes successfully")
    XCTAssertEqual(view1?.renderedImage, .none, "Expected no image state change for second view once first image loading completes successfully")
    
    let imageData1: Data = UIImage.make(withColor: .blue).pngData()!
  ...
  private func assertThat(_ sut: FeedViewController, isRendering feed: [FeedImage], file: StaticString = #filePath, line: UInt = #line) {
    guard sut.numberOfRendererFeedImageViews() == feed.count else {
      return XCTFail("Expected \(feed.count) images, got \(sut.numberOfRendererFeedImageViews()) instead", file: file, line: line)
    }
